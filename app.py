...
# ✅ ID FISSO DIMAGRIMENTO OVER 40
SPREADSHEET_ID = "1WMogxtwpnzDrL9ryoW71o05cCzWVlPItkfBNJqJW1Gk"

# 🔔 Webhook flusso 2 (Dimagrimento Over 40)
N8N_WEBHOOK_URL_FLUSSO2 = os.environ.get("N8N_WEBHOOK_URL_DIMAGRIMENTO_OVER_FLUSSO2")
...
def notify_n8n_flusso2(job):
    ...
    if not N8N_WEBHOOK_URL_FLUSSO2:
        print("⚠️ N8N_WEBHOOK_URL_DIMAGRIMENTO_OVER_FLUSSO2 non configurata, skip webhook", flush=True)
        return
...
# -------------------------------------------------
# Mapping SCENA → QUERY visiva (DIMAGRIMENTO OVER 40)
# -------------------------------------------------
def pick_visual_query(context: str, keywords_text: str = "") -> str:
    """Query B-roll DIMAGRIMENTO: wellness donna 40+, cucina sana, fitness."""
    ctx = (context or "").lower()
    kw = (keywords_text or "").lower()
    
    base = "woman 45 wellness lifestyle kitchen home healthy food fitness"
    
    # Pancia / Gonfiore / Digestione
    if any(w in ctx for w in ["pancia", "gonfio", "gonfia", "digestione", "intestino", "addominale"]):
        return "woman 45 bloated belly closeup bathroom measuring waist health"
    
    # Bilancia / Peso / Chili
    if any(w in ctx for w in ["bilancia", "peso", "kg", "chili", "dimagr"]):
        return "woman 45 stepping on scale bathroom weight loss measurement"
    
    # Alimentazione / Dieta / Pasti / Verdure
    if any(w in ctx for w in ["alimentazione", "dieta", "pasto", "verdure", "insalata", "cucina"]):
        return "woman 45 preparing healthy meal kitchen colorful vegetables cooking salad"
    
    # Acqua / Idratazione
    if any(w in ctx for w in ["acqua", "idrat", "bere"]):
        return "woman 45 drinking water kitchen daylight hydration wellness"
    
    # Allenamento / Yoga / Esercizio / Fitness
    if any(w in ctx for w in ["allenamento", "yoga", "esercizio", "fitness", "movimento"]):
        return "woman 45 home workout living room yoga mat energetic exercise"
    
    # Sonno / Riposo / Relax
    if any(w in ctx for w in ["sonno", "dormi", "riposo", "relax", "stanchez"]):
        return "woman 45 sleeping bedroom peaceful rest relaxation night routine"
    
    # Stress / Cortisolo / Ormoni
    if any(w in ctx for w in ["stress", "cortisol", "ormoni", "menopausa"]):
        return "woman 45 stressed tension headache fatigue hormones menopause symptoms"
    
    # Fame / Emotiva / Nervosa
    if any(w in ctx for w in ["fame", "emotion", "nervos", "voglia"]):
        return "woman 45 emotional eating craving kitchen fridge temptation stress eating"
    
    # Keywords da Sheet
    if kw and kw != "none":
        return f"{kw}, woman 45 wellness, healthy lifestyle, fitness kitchen, weight loss"
    
    # Fallback
    return base

def is_wellness_video_metadata(video_data, source):
    """Filtro PERMISSIVO Wellness: accetta tutto tranne animali/sport estremi."""
    wellness_keywords = ["woman", "female", "kitchen", "home", "wellness", "health", "fitness",
                        "food", "cooking", "yoga", "exercise", "lifestyle", "diet"]
    ...
    wellness_count = sum(1 for kw in wellness_keywords if kw in text)
    has_banned = any(kw in text for kw in banned)
    
    if has_banned:
        status = "❌ BANNED"
    elif wellness_count >= 1:
        status = f"✅ WELLNESS({wellness_count})"
    else:
        status = f"⚠️ NEUTRAL(wellness:{wellness_count})"
...
def fetch_clip_for_scene(scene_number: int, query: str, avg_scene_duration: float):
    """🎯 DIMAGRIMENTO: B-roll wellness donna 40+. Fallback Pixabay se Pexels 0."""
    ...
    def try_pexels():
        ...
        params = {
            "query": f"{query} woman wellness healthy lifestyle kitchen fitness",
        ...
        print(f"🎯 Pexels: {len(videos)} totali → {len(wellness_videos)} WELLNESS OK (no banned)", flush=True)
        if wellness_videos:
    ...
    def try_pixabay():
        ...
        params = {
            "key": PIXABAY_API_KEY,
            "q": f"{query} woman wellness healthy lifestyle kitchen fitness",
            "per_page": 25,
            "safesearch": "true",
            "min_width": 1280,
        }
...
def process_video_async(job_id, data):
    """Processa video DIMAGRIMENTO OVER 40 in background thread"""
    job = jobs[job_id]
    ...
    try:
        ...
        print("=" * 80, flush=True)
        print(f"🎬 START DIMAGRIMENTO OVER 40: {len(script)} char script, keywords: '{sheet_keywords}', row: {row_number}", flush=True)
        ...
        for i in range(num_scenes):
            ...
            scene_context = " ".join(script_words[word_index: word_index + 7]) if word_index < len(script_words) else "woman wellness healthy lifestyle"
        ...
        print(f"✅ 🔥 VIDEO DIMAGRIMENTO OVER 40 COMPLETO: {real_duration/60:.1f}min → {public_url}", flush=True)
        ...
        print(f"❌ ERRORE DIMAGRIMENTO: {e}", flush=True)
...
def generate():
    try:
        ...
        print(f"🚀 DIMAGRIMENTO OVER 40 Job {job_id} QUEUED: raw_row={data.get('row_number')}", flush=True)
        return jsonify({
            "success": True,
            "job_id": job_id,
            "status": "queued",
